...
server_messages_queue = {Config.ASTRAX_SERVER_TAG: [], Config.BERNUM_SERVER_TAG: []}
...
def login():
    ...
    if f.request.method == "POST":
        ...
        return (
            f.redirect(next_page) if next_page else f.redirect(f.url_for("main.index"))
        )
...
def view_external_messages():
    ...
    if filter_tag != "":
        data = (
            Extern_Messages.query.filter(Extern_Messages.tags.like(f"%#{filter_tag}$%"))
            .order_by(Extern_Messages.timestamp.desc())
            .paginate(per_page=20)
        )
    else:
        data = Extern_Messages.query.order_by(
            Extern_Messages.timestamp.desc()
        ).paginate(per_page=20)
...
def server_restart_callback(message: Extern_Messages) -> str:
    """Handle external message when Config.SERVER_RESTART_TAG is present in the
    tags. Function reports to user.
...
def log_external_message():
    """End point for external messages such as local server logs and local
    ...
    item = Extern_Messages(
        id=id, user=user, message=message, timestamp=timestamp, tags=tags
    )
...
def url_redirects():
    ...
    if f.request.method == "GET":
        ...
        return f.render_template(
            "admin/url-redirects.jinja", title="URL Redirects", urls=urls
        )
